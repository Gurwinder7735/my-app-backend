...
module.exports = {
  claimOwnership: catchAsync(async (req, res, next) => {

    // const watchId = req.params.watchId.split("-")[1];
    const {watchId} = req.params;
    const {verificationCode} = req?.body
    


    const watch = await fetchWatchDetail(watchId || "");
    ...
    if (watch?.owner?.id) {
      return next(
        new AppError(
          STATUS_MSG.ERROR.OWNERSHIP_ALREADY_CLAIMED,
          STATUS_CODES.BAD_REQUEST
        )
      );
    };

    if(watch?.verificationCode !== verificationCode){
      return next(
        new AppError(
          STATUS_MSG.ERROR.INVALID_VERIFICATION_CODE,
          STATUS_CODES.BAD_REQUEST
        )
      );
    }
    ...
    await updateWatchOwner({
      watchId: watch?.id,
      newOwnerId: req?.auth?.id,
    });
    
     sendOwnershipClaimedMailToBrand(watch,req.auth)
